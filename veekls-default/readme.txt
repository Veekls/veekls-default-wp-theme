=== VeeklsDefaultTheme ===
Contributors: Veekls
Requires at least: WordPress 4.5
Tested up to: WordPress 4.9.8
Tags: custom-menu, custom-logo, editor-style, featured-images, one-column, right-sidebar, rtl-language-support, threaded-comments, translation-ready, two-columns, grid-layout, microformats, sticky-post, theme-options, blog, e-commerce

== Description ==

VeeklsDefaultTheme is a beautiful and easy to use WordPress Veekls theme for car sellers and dealers. Working perfectly with Veekls plugin, this theme helps you build a beautiful car listings WordPress site with ease. Built on the latest technology of WordPress, VeeklsDefaultTheme is fully responsvive, fast and optimized for all mobile phones.

== Installation ==

Please see https://wpautolistings.com/docs/veekls-default-theme/

== License ==

Unless otherwise specified, all the theme files, scripts and images are licensed under GNU General Public License Version 3.
License: GNU General Public License v3 or later
License URI: http://www.gnu.org/licenses/gpl-3.0.html
VeeklsDefaultTheme WordPress Theme, Copyright 2018 Veekls
VeeklsDefaultTheme is distributed under the terms of the GNU GPL

Unless otherwise specified, all images are self shot by Veekls
Copyright: Veekls, https://wpautolistings.com/

This program is free software: you can redistribute it and/or modify
it under the terms of the GNU General Public License as published by
the Free Software Foundation, either version 3 of the License, or
(at your option) any later version.

This program is distributed in the hope that it will be useful,
but WITHOUT ANY WARRANTY; without even the implied warranty of
MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
GNU General Public License for more details.

IcoFont icons, Copyright IcoFont
License: SIL Open Font License
Source: http://icofont.com/

Slick Slider, Copyright Ken Wheeler
License: MIT
Source : http://kenwheeler.github.io/slick/

Jquery.sumoselect, Copyright Hemant Negi
License: MIT
Source : https://github.com/HemantNegi/jquery.sumoselect

Hybrid Core, Copyright Justin Tadlock
License: GNU General Public License v2.0
Source : https://github.com/justintadlock/hybrid-core/blob/master/license.md

TGM Plugin Activation, Copyright TGMPA
License: GNU General Public License v2.0
Source : https://github.com/TGMPA/TGM-Plugin-Activation/blob/develop/LICENSE.md

Screenshot Image, Copyright PxHere
License: CC0 1.0 Universal (CC0 1.0)
Source: https://pxhere.com/en/photo/67951

VeeklsDefaultTheme is based on Underscores http://underscores.me/, (C) 2012-2018 Automattic, Inc.
Underscores is distributed under the terms of the GNU GPL v2 or later.

VeeklsDefaultTheme is Licensed under the GNU GPL 3 license
https://wpautolistings.com/


== Changelog ==
= 1.0.5 =
* Update license for image

= 1.0.4 =
* Remove unnecessary files and code.

= 1.0.3 =
* Update demo content

= 1.0.2 =
* New dashboard and change some options for footer

= 1.0.1 =
* Fix style on some elements

= 1.0.0 =
* First submit

